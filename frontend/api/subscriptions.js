const BASE_URL = process.env.NEXT_PUBLIC_API_URL || "https://case-task-9kv7.onrender.com/api";
...
export const getSubscriptions = async (email) => {
  const res = await fetch(`${BASE_URL}/subscriptions?email=${encodeURIComponent(email)}`);
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || "Something went wrong");
